...
public final class HorizonsBlockFamilies {
    ...
    public static final BlockFamily JACARANDA_PLANKS_FAMILY = new BlockFamily.Builder(JACARANDA_PLANKS.get()).button(JACARANDA_BUTTON.get()).fence(JACARANDA_FENCE.get()).fenceGate(JACARANDA_FENCE_GATE.get()).pressurePlate(JACARANDA_PRESSURE_PLATE.get()).sign(JACARANDA_SIGNS.getFirst().get(), JACARANDA_SIGNS.getSecond().get()).slab(JACARANDA_SLAB.get()).stairs(JACARANDA_STAIRS.get()).door(JACARANDA_DOOR.get()).trapdoor(JACARANDA_TRAPDOOR.get()).recipeGroupPrefix("wooden").recipeUnlockedBy("has_planks").getFamily();
