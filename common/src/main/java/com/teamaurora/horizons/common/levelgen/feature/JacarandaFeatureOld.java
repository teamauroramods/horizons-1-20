...
import com.teamaurora.horizons.common.util.DirectionalBlockPos;
import com.teamaurora.horizons.common.util.TreeUtil;
...
public class JacarandaFeatureOld extends Feature<TreeConfiguration> {

    public JacarandaFeatureOld(Codec<TreeConfiguration> config) {
        super(config);
    }
...
//        }
    ...
    private void addBranch(BlockPos pos, Direction dir, List<BlockPos> leaves, List<DirectionalBlockPos> logs, RandomSource rand) {
    ...
    private void addCanopy(BlockPos pos, List<BlockPos> leaves, RandomSource rand) {
    ...
    private void cir1(BlockPos pos, List<BlockPos> leaves, RandomSource rand) {
        for (int x = -1; x <= 1; x++) {
            for (int z = -1; z <= 1; z++) {
    ...
    private void cir2(BlockPos pos, List<BlockPos> leaves, RandomSource rand) {
        for (int x = -2; x <= 2; x++) {
            for (int z = -2; z <= 2; z++) {
