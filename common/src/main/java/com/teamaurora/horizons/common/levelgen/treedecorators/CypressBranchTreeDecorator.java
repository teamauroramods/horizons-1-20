...
public class CypressBranchTreeDecorator extends TreeDecorator {
    ...
    public static final Codec<CypressBranchTreeDecorator> CODEC = Codec.unit(() -> INSTANCE);
        ...
        for (BlockPos pos : context.logs())
            if (random.nextInt(12) == 0) {
                ...
                if (context.isAir(posToUse))
                    context.setBlock(posToUse, HorizonsBlocks.CYPRESS_BRANCH.get().defaultBlockState()
                            .setValue(CypressBranchBlock.FACING, dir).setValue(CypressBranchBlock.AGE, 2));
            }
    }
